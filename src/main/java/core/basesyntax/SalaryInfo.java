...
public class SalaryInfo {
    ...
    public String getSalaryInfo(String[] names, String[] data, String dateFrom, String dateTo) {
        ...
        int[] countedSalary = extracted(names, from, to, list);
        return formReport(names, dateFrom, dateTo, countedSalary);
    }

    private int[] extracted(String[] names, LocalDate from, LocalDate to, List<String> list) {
        int[] countedSalary = new int[names.length];
        LocalDate dateToCompare;
        for (String s : list) {
            String[] strings = s.split("\\s");
            dateToCompare = LocalDate.parse(strings[0], DATE_TIME_FORMATTER);
            if (isMatchedDate(from, to, dateToCompare)) {
                for (int j = 0; j < names.length; j++) {
                    if (names[j].contains(strings[1])) {
                        countedSalary[j] += multiplyHoursOnRate(strings);
                    }
                }
            }
        }
    ...
    private boolean isMatchedDate(LocalDate from, LocalDate to, LocalDate dateToCompare) {
        return dateToCompare.compareTo(from) >= 0
                && dateToCompare.compareTo(to) <= 0;
    }

    private String formReport(String[] names, String dateFrom, String dateTo, int[] countedSalary) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(String.format("Report for period %s - %s", dateFrom, dateTo))
                .append(System.lineSeparator());
        for (int i = 0; i < names.length; i++) {
            stringBuilder.append(names[i]).append(" - ").append(countedSalary[i])
                    .append(System.lineSeparator());
        }
