...
public class SalaryInfo {
    public String getSalaryInfo(String[] names, String[] data, String dateFrom, String dateTo) {
        StringBuilder reportText = new StringBuilder("Report for period " + dateFrom + " - " + dateTo);
            ...
            if (reportDate.checkIsDateInRange(dateFrom, dateTo)) {
                for (User user : usersList) {
                    if (user.getName().equals(userReport.getName())) {
                        user.addMoneyToSalary(userReport.getSalaryPerHour() * userReport.getHoursCount());
                        break;
                    }
                }
            }
