...
public class SalaryInfo {
    ...
    public String getSalaryInfo(String[] names, String[] data, String dateFrom, String dateTo) {
            ...
            for (int i = 0; i < data.length; i++) {
                ...
                if (name.equals(info[1])) {
                    if ((actualDate.after(from) || actualDate.equals(from))
                            && (actualDate.before(endDate) || actualDate.equals(endDate))) {
        ...
        StringBuilder res = new StringBuilder(RETURN_ROW + dateFrom + HYPHEN + dateTo);
        for (String[] rows: secondArray) {
            res.append(System.lineSeparator()).append(rows[0]).append(HYPHEN).append(rows[1]);
        }
        return res.toString();
    }
}
