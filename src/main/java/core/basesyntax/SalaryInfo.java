    ...
    public String getSalaryInfo(String[] names, String[] data, String dateFrom, String dateTo) {
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            for (int j = 0; j < data.length; j++) {
                ...
                LocalDate workingDay = LocalDate.of(Integer.valueOf(workD[2]), Integer.valueOf(workD[1]), Integer.valueOf(workD[0]));
                if (!(workingDay.getDayOfYear() >= startToWork.getDayOfYear() && workingDay.getDayOfYear() <= endToWork.getDayOfYear())) {
                    continue;
                }
